...
// AEAD interface: SIPO input loading, core control, PISO output multiplexing.
...
module aead_itf#(
                ...
                (
                  ...
                  output wire [WIDTH-1:0]           o_end_op,                 //-- End Operation Signal (contains trigger for CW)
                  output wire                       o_busy,                   //-- Sets to 1 when AEAD operation begins (register output)
                  output wire [OUT_REG*WIDTH-1:0]   o_result_digest           //-- For shadow registers!
                );
	...
	aead_core_top #(.PIPELINE_STAGES(PIPELINE_STAGES), 
